...
function filterSearch(val){const q=val.toLowerCase().trim();if(!q){render();return;}uid=0;const h=sorted(RAW.workbooks,'workbookSort','totalRows').map(w=>renderFW(w,q)).join('')+sorted(RAW.folders,'folderSort','totalRows').map(f=>renderFF(f,q)).join('');document.getElementById('main').innerHTML=h||'<div class="empty-msg" style="padding:20px;font-size:13px;color:#9ca3af">No results</div>';}
function countAll(n){let w=n.workbooks.length,t=n.workbooks.reduce((s,wb)=>s+wb.tables.length,0);n.folders.forEach(f=>{const c=countAll(f);w+=c.w;t+=c.t;});return{w,t};}
function render(){uid=0;const rw=sorted(RAW.workbooks,'workbookSort','totalRows'),rf=sorted(RAW.folders,'folderSort','totalRows'),rwt=rw.reduce((s,w)=>s+w.totalRows,0),gt=rwt+RAW.folders.reduce((s,f)=>s+f.totalRows,0),c=countAll(RAW);document.getElementById('statsRow').innerHTML=\`<div class="stat-card"><div class="stat-label">Total Rows</div><div class="stat-val">\${fmt(gt)}</div><div class="stat-sub">across all workbooks</div></div><div class="stat-card"><div class="stat-label">Folders</div><div class="stat-val">\${RAW.folders.length}</div><div class="stat-sub">top-level</div></div><div class="stat-card"><div class="stat-label">Workbooks</div><div class="stat-val">\${fmt(c.w)}</div><div class="stat-sub">total</div></div><div class="stat-card"><div class="stat-label">Tables</div><div class="stat-val">\${fmt(c.t)}</div><div class="stat-sub">total</div></div>\`;let html='';if(rw.length){const id=nid();html+=\`<div class="folder-block"><div class="folder-row" onclick="tog('\${id}')"><span class="chevron" id="c\${id}">▶</span><span class="folder-emoji">🗂️</span><span class="folder-label" style="flex:1">(No folder)</span><span class="folder-wbcount">\${rw.length} wb</span><div style="width:90px;margin:0 8px"></div><span class="folder-rowcount">\${fmt(rwt)}</span></div><div class="folder-children" id="\${id}">\${rw.map(renderWorkbook).join('')}</div></div>\`;}html+=rf.map(renderFolder).join('');document.getElementById('main').innerHTML=html;document.getElementById('sidebarFolders').innerHTML=rf.map(f=>\`<a class="sidebar-item" href="\${BASE}/home/\${f.id}" target="_blank"><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">\${esc(f.name)}</span><span style="font-size:10px;color:#d1d5db;flex-shrink:0">\${fmt(f.totalRows)}</span></a>\`).join('');}
render();
</script>
</body>
</html>`;
